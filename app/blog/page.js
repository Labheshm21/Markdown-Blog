...
/**
 * This is an async function that fetches blog data on the server side
...
const Blog = async () => {
  ...
  // Read all the files in the content directory
  const dirContent = fs.readdirSync(contentDirectory, 'utf-8');
  
  // Read the content of each file and parse the front matter using gray-matter
  const blogs = dirContent.map((file) => {
    const filePath = path.join(contentDirectory, file);
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    const { data } = matter(fileContent);  // Extract front matter

    return data;  // Return only the front matter for the blog post
  });

  return (
    <div className="container mx-auto p-4">
      {/* Main heading for the blog section */}
      <h1 className="text-4xl font-bold mb-8 text-center">New Blogs</h1>
